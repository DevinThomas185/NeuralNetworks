...
def linear():
    ...
    grad_z = np.array([[1, -3, 4, 1.2], [0.6, -2.7, 3.3, -0.3],
                      [1.2, -5.2, 1.9, 2.4], [2.8, -2.9, 5.1, -1.2]])
...
def activation():
    ...
    assert (grad_x.shape == grad_z.shape)
    print(grad_x)
    assert (np.isclose(grad_x, grad_z *
            (layer._cache_current > 0).astype(int)).all())


if __name__ == "__main__":
    print("1 - Linear Layer Test\n2 - Activation Layer Test")
    option = input()
    if option == "1":
        linear()
    elif option == "2":
